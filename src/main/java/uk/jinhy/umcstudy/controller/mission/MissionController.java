...
public class MissionController {
    ...
    @Operation(summary = "특정 레스토랑 미션 불러오기 API",description = "특정 레스토랑의 미션을 불러오는 API이며, 페이징을 포함합니다. path variable로 restaurant의 id를, query String 으로 page 번호를 주세요")
    @ApiResponses({
    ...
    @Parameters({
            @Parameter(name = "page", description = "불러올 미션의 페이지 번호"),
            @Parameter(name = "id", description = "불러올 미션의 레스토랑 아이디")
    })
    public ApiResponse<MissionResponseDTO.GetRestaurantMissionDTO> getRestaurantMission(
        ...
        Page<UserMission> missionPage = missionQueryService.getRestaurantMission(id, page - 1, 10L);
        return ApiResponse.onSuccess(missionConverter.toGetRestaurantMissionDto(missionPage));
    }

    @GetMapping("/user/proceed")
    @Operation(summary = "유저 미션 불러오기 API",description = "유저의 진행중인 미션을 불러오는 API이며, 페이징을 포함합니다. query String 으로 page 번호를 주세요")
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "COMMON200",description = "OK, 성공"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "AUTH003", description = "access 토큰을 주세요!",content = @Content(schema = @Schema(implementation = ApiResponse.class))),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "AUTH004", description = "acess 토큰 만료",content = @Content(schema = @Schema(implementation = ApiResponse.class))),
    ...
    public ApiResponse<MissionResponseDTO.GetMyInProgressMissionsDTO> getMyInProgressMissions(
            @PageNumber @RequestParam(value = "page") Long page
    ) {
        Page<UserMission> missionPage = missionQueryService.getMyInProgressMission("Test", page - 1, 10L);
        return ApiResponse.onSuccess(missionConverter.toGetMyInProgressMissionsDto(missionPage));
    }

    @PatchMapping("/{id}/complete")
    @Operation(summary = "미션 완료 API",description = "유저의 진행중인 미션을 불러오는 API이며, 페이징을 포함합니다. query String 으로 page 번호를 주세요")
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "COMMON200",description = "OK, 성공"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "AUTH003", description = "access 토큰을 주세요!",content = @Content(schema = @Schema(implementation = ApiResponse.class))),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "AUTH004", description = "acess 토큰 만료",content = @Content(schema = @Schema(implementation = ApiResponse.class))),
    ...
    @Parameters({
            @Parameter(name = "id", description = "완료할 미션의 아이디")
    })
    public ApiResponse<MissionResponseDTO.CompleteMissionDTO> completeMission(
            @PathVariable(value = "id") Long id
    ) {
        UserMission mission = missionCommandService.completeMission(id);
        return ApiResponse.onSuccess(missionConverter.toCompleteMissionDto(mission));
    }
}
